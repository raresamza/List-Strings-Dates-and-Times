...
string text2 = "Hello my name. is rares";
...
string[] wordCout=text.Split(' ');
Console.WriteLine($"There are {wordCout.Length} words in the string.");
string[] sentanceCount=text.Split('.');
Console.WriteLine($"There are {sentanceCount.Length} sentances in this string");
int encapsulationCOunt = 0;
foreach(string word in wordCout)
{
    if(word.ToLower().Contains("encapsulation")) {  encapsulationCOunt++; }
}
Console.WriteLine($"There are {encapsulationCOunt} instances of the word \"encapsulation\"");
...
Console.WriteLine();
Console.WriteLine("After replacing hte string is:");
